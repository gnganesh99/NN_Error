#!/usr/bin/env python3
"""
Notebook-style BEPS active learning script:
- Loads FORC_BEPS_1d8um_0010.h5
- Prepares image/spectra pairs
- Runs iterative active learning loop
- Saves results to HDF5
- Plots acquired points
"""
...
from nnerror.training_functions import err_estimation, train_model, predict_spectra, norm_0to1, predict_posterior, append_training_set, distance_acq_fn
...
beps_url = (
    "https://www.dropbox.com/scl/fi/syjh9jp0mn9pturbjgs82/FORC_BEPS_1d8um_0010.h5?rlkey=27kndbclb9pxapjic1t62uyak&dl=0"
)

data_folder = REPO_ROOT / "data" / "BEPS"
output_folder = REPO_ROOT / "data" / "beps_active_learning_1.8"
...
def parse_args():
    ...
    parser.add_argument("--beps-filename", default="FORC_BEPS_1d8um_0010.h5")
...
class Norm_0to1:
    def __init__(self, data):
        ...
        self.data_range = max(self.max_val - self.min_val, 1e-8)
...
def main():
    ...
    model_types = ["im2spec", "FNO", "im2spec_attn", "FNO_attn"]
    acquisition_methods = ["distance", "random"]

    for model_type in model_types:
        for acquisition_method in acquisition_methods:
            print(f"\nRunning active learning with model_type: {model_type}, acquisition_method: {acquisition_method}")
        output_dict = {
            "reduced_errors": [],
        ...
        for iteration in range(acquisition_iterations):
            ...
            print(f"\nActive Learning Iteration {iteration + 1}/{acquisition_iterations}")
            ...
            # -------------------------
            # Plot acquired points
            # -------------------------
            initial_train_indices = output_dict["train_indices"][0]
            final_train_indices = output_dict["train_indices"][-1]

            # Indices acquired during active learning
            acquired_indices = np.setdiff1d(final_train_indices, initial_train_indices)

            # Coordinates of acquired points: use reduced_coords from the last iteration
            reduced_coords = output_dict["reduced_coords"][-1]

            # Map acquired indices to their coordinates
            # Assuming reduced_coords is aligned with original_indices order
            ...
            plt.scatter(
                acquired_coords[:, 1],   # x-coordinate
                acquired_coords[:, 0],   # y-coordinate
                c="red",
                s=40,
                edgecolors="white",
                linewidths=0.8,
